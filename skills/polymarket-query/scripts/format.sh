#!/bin/bash
#
# Format Polymarket API responses into readable text

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/common.sh"

# Format a wallet address (truncated display)
...
# Format a P&L amount (with +/- sign and thousands separators)
...
format_volume() {
    ...
    # Use awk to handle floating-point numbers
    echo "$vol" | awk '{
...
# Format event detail
# Input: JSON array (single event, from stdin)
format_event_detail() {
    jq -r '
        .[0] // empty |
        "📌 \(.title)\n",
...
format_leaderboard() {
    ...
    echo "$input" | jq -r '
        ...
        ) | Volume: $\(.vol | . * 100 | round / 100 | tostring)",
        "   🔑 \(.proxyWallet | "\(.[0:6])…\(.[-4:])")",
        ""
    '
}
...
format_positions() {
    ...
    echo "$input" | jq -r '
        ...
        " Size: \(.size // 0 | . * 100 | round / 100) | Value: $\(.currentValue // 0 | . * 100 | round / 100)",
        "   💰 P&L: \(
...
format_trades() {
    ...
    echo "$input" | jq -r '
        ...
        " | \(.title)" +
        (if .outcome then " [\(.outcome)]" else "" end),
        "   💵 Size: \(.size // 0 | . * 100 | round / 100) @ $\(.price // 0 | . * 10000 | round / 10000)" +
        " | 🕐 \(.timestamp // 0 | todate | split("T") | .[0] + " " + (.[1] | split("Z") | .[0] | .[0:5]) + " UTC")",
        ""
    '
}

# Format the historical price table
# Input: JSON array (each item contains at least timestamp/date and price/value)
...
# Format the trend summary (start/end/absolute change/relative change)
# Input: JSON array (in chronological order)
format_trend_summary() {
    ...
    echo "End: $end_fmt"
    echo "Absolute change: $abs_change"
    echo "Relative change: $rel_change"
}

# Format the volume trend table
# Input: JSON array (each item contains at least timestamp/date and volume/value)
