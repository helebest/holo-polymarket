...
parse_series_command_args() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            --format)
                if [ -z "${2:-}" ]; then
                    echo "Argument error: --format requires a value"
                    return 1
                fi
                SERIES_FORMAT="$2"
                shift 2
            ...
            --out)
                if [ -z "${2:-}" ]; then
                    echo "Argument error: --out requires a value"
                    return 1
                fi
                SERIES_OUT="$2"
                shift 2
    ...
    if [ "${#positional[@]}" -lt 3 ] || [ "${#positional[@]}" -gt 4 ]; then
        echo "Usage: bash polymarket.sh ${cmd_name} <event-slug> <from> <to> [interval] [--format csv|json] [--out file-path]"
        return 1
    fi
    ...
    if [ -n "$SERIES_OUT" ] && [ -z "$SERIES_FORMAT" ]; then
        echo "Argument error: --out must be used together with --format"
        return 1
    fi

    if [ -n "$SERIES_FORMAT" ] && ! validate_export_format "$SERIES_FORMAT"; then
        echo "Invalid format: only csv/json are supported"
        return 1
    fi
...
run_series_command() {
    ...
    if ! validate_time_range "$SERIES_FROM_DATE" "$SERIES_TO_DATE"; then
        echo "Invalid time range: from/to must be YYYY-MM-DD and from <= to"
        return 1
    fi

    if ! validate_interval "$SERIES_INTERVAL"; then
        echo "Invalid interval: only 1h/4h/1d are supported"
        return 1
    fi
...
handle_series_command() {
    ...
    case "$cmd" in
        history)
            run_series_command "history" "price" "format_price_history_table" "🕒 Price history" "$@"
            ;;
        trend)
            run_series_command "trend" "price" "format_trend_summary" "📈 Probability trend" "$@"
            ;;
        volume-trend)
            run_series_command "volume-trend" "volume" "format_volume_trend_table" "📊 Volume trend" "$@"
            ;;
        *)
            return 2
            ;;
